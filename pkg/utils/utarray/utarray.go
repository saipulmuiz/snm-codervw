...
// IsIntersect checks if two params is intersected.
//
// Example:
// var roles = []string{"rm", "head_rm"}
// var allowedRoles = []string{"head_ca", "ca"}
//
//	if !IsIntersect(roles, allowedRoles){
//			return errors.New("forbidden roles!")
//		}
//
// .....
func IsIntersect(a, b interface{}) bool {

	smallerSet := convertToSliceInterface(a)
	largerSet := convertToSliceInterface(b)

	// Create a map to store elements of the first slice

	elements := make(map[interface{}]bool)

	if len(*smallerSet) > len(*largerSet) {
		tempSet := smallerSet
...
// Helper function to convert to slice if necessary
func convertToSliceInterface(v interface{}) *[]interface{} {
